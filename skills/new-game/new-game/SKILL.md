    ...
    "PreCompact": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cp \"$CLAUDE_PROJECT_DIR/.planning/STATE.md\" \"$CLAUDE_PROJECT_DIR/.planning/STATE.md.bak\" 2>/dev/null; cp \"$CLAUDE_PROJECT_DIR/.planning/ROADMAP.md\" \"$CLAUDE_PROJECT_DIR/.planning/ROADMAP.md.bak\" 2>/dev/null; echo 'Planning files backed up before auto-compaction'"
          }
        ]
      }
    ],
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "BRANCH=$(git -C \"$CLAUDE_PROJECT_DIR\" branch --show-current 2>/dev/null); if [ \"$BRANCH\" = \"main\" ] || [ \"$BRANCH\" = \"master\" ]; then echo 'BLOCKED: Cannot edit on main/master. Create a feature branch first.' >&2; exit 2; fi"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if [ -f \"$CLAUDE_PROJECT_DIR/package.json\" ] && grep -q '\"test:run\"' \"$CLAUDE_PROJECT_DIR/package.json\" && echo \"$CLAUDE_FILE_PATH\" | grep -qE '\\.(ts|tsx|js|jsx)$'; then cd \"$CLAUDE_PROJECT_DIR\" && npm run test:run 2>&1 | tail -20; fi"
          }
        ]
      }
    ]
...
You now have:
  - Lossless hooks (SessionStart, PreCompact, Stop, PreToolUse, PostToolUse)
