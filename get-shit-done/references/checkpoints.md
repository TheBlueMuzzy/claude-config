...
2. **Prepare checkpoint context:**
   - Read `version.json` for current version (if exists)
   - For web projects: ensure dev server is running with `--host` flag, capture localhost + network URLs
   - Gather all info before displaying
3. **Display checkpoint clearly:**
...
**URL display rule:** URLs go on their own line with NO inline label prefix. This lets the user double-click to select the full URL for easy copying. Labels go ABOVE the URLs on a separate line.

4. **Wait for user response** - do not hallucinate completion
5. **Verify if possible** - check files, run tests
6. **Resume execution** - continue only after confirmation
