...
**Before displaying checkpoint:**

1. **Version:** Read `version.json` from project root (if exists). Extract current version string (X.Y.Z.B format).
2. **Dev server (web projects only):** If `package.json` exists with a `dev` script:
   - **Always start from PROJECT_DIR** — never assume an existing server on any port belongs to this project (multiple projects may be running)
   - Start: `cd PROJECT_DIR && npm run dev -- --host` in background (let Vite auto-find an available port)
   - Wait 3-4 seconds for server to start
   - Capture the actual localhost URL and network URL from the Vite output (parse the `Local:` and `Network:` lines)
   - **Never hardcode port 5173** — always use the port Vite reports
3. **Display:** Include version and server URLs in the checkpoint block

**URL display rule:** URLs must be on their own line with NO inline label prefix. This lets the user double-click to select the full URL and copy it instantly. Put a label ABOVE the URLs on a separate line, not inline.
...
```
**What was built:**
[what was automated - deployed, built, configured]

**How to verify:**
1. Open the server URL above on your phone (or browser with camera)
...
**MANDATORY PATH VERIFICATION — before presenting any "Next Up" command:**

```bash
# Construct the exact path you're about to recommend, then verify it exists
NEXT_PLAN=".planning/phases/[phase-dir]/[phase]-[next-plan]-PLAN.md"
ls "$NEXT_PLAN" 2>/dev/null && echo "PATH_VALID" || echo "PATH_INVALID"
```

**If PATH_INVALID:** Do NOT present a copy-paste command. Instead show:
```
Next: Run `/gsd:progress` to route to the next action.
```

**If PATH_VALID:** Proceed with the "Next Up" block below using the verified path.
...
**MANDATORY PATH VERIFICATION — verify next phase directory exists:**

```bash
ls -d .planning/phases/[next-phase-number]-*/ 2>/dev/null && echo "PHASE_DIR_VALID" || echo "PHASE_DIR_INVALID"
```

**If PHASE_DIR_INVALID:** The next phase hasn't been planned yet. Show:
```
Next: Run `/gsd:progress` to route to the next action.
```
Do NOT fabricate a phase directory name. **If PATH_VALID:** Proceed with the block below.
