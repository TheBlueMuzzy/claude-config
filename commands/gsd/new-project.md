...
<step name="prd_bridge">

**Check if BMUZ Double Diamond already completed:**

```bash
[ -f .planning/PRD.md ] && echo "PRD_EXISTS" || echo "NO_PRD"
```

**If PRD_EXISTS:**
The Double Diamond design process already captured everything we need. Follow `~/.claude/get-shit-done/lib/prd-to-project.md` to auto-generate PROJECT.md from the PRD.

After generation:
- Skip directly to the **mode** step (no questioning needed)
- Show: `Generated PROJECT.md from PRD.md — skipping questions (Double Diamond already answered them)`

**If NO_PRD:** Continue to brownfield_offer step (normal questioning flow).
...
<step name="breadcrumb">

**Update active project breadcrumb** so GSD commands can find this project from any directory:
